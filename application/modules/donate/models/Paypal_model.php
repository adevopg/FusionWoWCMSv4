...
class Paypal_model extends CI_Model
{
    public function __construct()
    {
        ...
        Stripe::setApiKey($this->config->item('stripe_secret_key'));
		 ...
		 public function getStripeDonations()
    {
        $query = $this->db->get('stripe_donate');
        return $query->result();
    }
	
	 public function insertStripeLog($data)
    {
        // Inserta un registro en la tabla stripe_logs
        $this->db->insert('stripe_logs', $data);

        // Devuelve el ID del registro insertado
        return $this->db->insert_id();
    }
	
	public function getStripeDonationAmount($dataId)
    {
        // Consulta la base de datos para obtener el monto de stripe_donate según la ID del producto
        ...
        if ($query->num_rows() > 0) {
            // Devuelve el monto obtenido
            return $query->row()->price;
        } else {
            // Maneja la situación donde no se encontró el producto
            // Puedes devolver un valor predeterminado o lanzar una excepción según tu lógica
            return 0.50; // Devuelve 0.50 euros por defecto
        }
    }
	...
	public function getStripeDonationData($dataId)
    {
        // Obtener los datos de la donación según la ID del producto
        $query = $this->db->query("SELECT * FROM stripe_donate WHERE id = ?", array($dataId));

        // Verificar si se encontraron datos
        if ($query->num_rows() > 0) {
            // Devolver la primera fila como objeto
            return $query->row();
        } else {
            // En este caso, no se encontraron datos
            return null;
        }
    }
